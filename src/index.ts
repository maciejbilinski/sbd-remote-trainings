  ...
  app.post('/delete-account', checkLoggedIn, async (req: Request, res: Response) => {
    ...
    if (typed_username !== username) {
      ...
      try {
        ...
        await connection.execute(`DELETE FROM obciazenia WHERE wykonanecwiczenia_id = (SELECT id FROM wykonanecwiczenia WHERE wykonanetreningi_id IN (SELECT id FROM wykonanetreningi WHERE plantreningowy_id IN (SELECT id FROM plantreningowy WHERE uzytkownicy_login=:login)))`, [username], { autoCommit: false });
        await connection.execute(`DELETE FROM wykonanecwiczenia WHERE wykonanetreningi_id IN (SELECT id FROM wykonanetreningi WHERE plantreningowy_id IN (SELECT id FROM plantreningowy WHERE uzytkownicy_login=:login))`, [username], { autoCommit: false });
        await connection.execute(`DELETE FROM wykonanetreningi WHERE plantreningowy_id IN (SELECT id FROM plantreningowy WHERE uzytkownicy_login=:login)`, [username], { autoCommit: false });
        ...
        await connection.execute(`DELETE FROM dnitreningowe WHERE plantreningowy_id IN (SELECT id FROM plantreningowy WHERE uzytkownicy_login=:login)`, [username], { autoCommit: false });
